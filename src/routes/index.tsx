...
function getInitialState(): AppState {
  ...
  const savedSystem = localStorage.getItem(SYSTEM_STORAGE_KEY) as MeasureSystem | null;

  const parsedRecipes: any[] = savedRecipes ? JSON.parse(savedRecipes) : [];

  const migratedRecipes: Recipe[] = parsedRecipes.map((r) => {
    const updated = { ...r };
    if (updated.instructions && updated.instructions.length > 0 && typeof updated.instructions[0] === 'string') {
      updated.instructions = (updated.instructions as unknown as string[]).map((text) => ({
        text: text,
        ingredients: [],
      }));
    }
...
const RecipeListSkeleton = () => (
  <div className="space-y-6">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {[1, 2, 3].map((i) => (
        <div key={i} className="bg-white rounded-2xl shadow-sm border border-cream-200 overflow-hidden">
        ...
        <div className="max-w-6xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            >
              ...
              <span>{state.measureSystem === 'metric' ? t.metric : t.imperial}</span>
            </button>
            <button
              type="button"
