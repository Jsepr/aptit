...
const buildSystemInstruction = ({
	...
	return [
		...
		'3. Provide the "originalIngredients" exactly from the source, using the same section structure (title + ingredients array).',
		`4. Translate to ${responseLanguage}.`,
		"5. For the ingredients list: Extract the main ingredients with their total amounts for the entire recipe. If the source has multiple sections (e.g. 'For the dough', 'For the filling', 'Topping'), use an array of objects with \"title\" (section name, translated) and \"ingredients\" (array of ingredient objects). If there is only one list with no section heading, use a single section with title \"\" or \"Ingredients\".",
		'6. For instruction steps: Include the exact same steps as in the source, but translated to the target language. If the source has multiple instruction sections, include a "title" on each step with that translated section heading. Omit "title" when there is only one section.',
